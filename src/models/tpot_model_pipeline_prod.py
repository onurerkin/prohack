...
from src.features.preprocess import preprocess_train_with_pred_opt, preprocess_train_with_pred_opt_prod
...
ds = preprocess_train_with_pred_opt_prod(standardize_or_not=True, impute_or_not=True)
...
full_ds = ds.X_train.append(ds.X_test)
...
ds.X_val = ds.X_train
ds.y_val = ds.y_train
# ds.X_train.dtypes[ds.X_train.dtypes==bool]
#
#
# ds.X_train = np.array(ds.X_train.astype('float64'))
# ds.X_test = np.array(ds.X_test.astype('float64'))
#
# ds.X_train = np.array(list(ds.X_train), dtype=np.float)
# ds.X_test = np.array(list(ds.X_test), dtype=np.float)
#
# ds.y_train = np.array(ds.y_train, dtype=np.float)
# ds.y_test = np.array(ds.y_test, dtype=np.float)

path_to_export = '/Users/onurerkinsucu/Dev/prohack/models/tpot_models/tpot_trial_4.py'

tpot_params = TPOTParams(dataset=ds, path_to_export=path_to_export, scoring='neg_log_loss', generations=20,
                         custom_validation=False, population_size=10)
model, prediction = train_tpot_classifier(tpot_params)

preds = pd.DataFrame(model.predict_proba(ds.X_test), columns=['p0','p1'])
preds['p1'].sum()

train_with_pred_opt = pd.read_csv('/Users/onurerkinsucu/Dev/prohack/data/interim/train_with_pred_opt.csv')
test_pred = np.array(preds[['p1']])

X_test_pred = ds.X_test
X_test_pred['p1'] = test_pred
X_test_pred.to_csv('data/processed/X_test_pred_tpot_31_may_binary.csv', index=False)