...
pred_test=pred_2.rename(columns={'predict':'y_pred'})
X_test_pred = pd.concat([ds.X_test.reset_index(drop=True), pred_test], axis=1)
X_test_pred.to_csv('data/processed/X_test_pred_h2o_23_may.csv', index=False)



pred_train=pred_train.rename(columns={'predict':'y_pred'})
X_train_pred = pd.concat([ds.X_train.reset_index(drop=True), pred_train], axis=1)
X_train_pred.to_csv('data/processed/X_train_pred_h2o_23_may.csv', index=False)
