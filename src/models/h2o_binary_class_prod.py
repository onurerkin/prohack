...
pred_test = h2o.as_list(saved_model.predict(hf_test))

pred_train = h2o.as_list(saved_model.predict(hf))

sum(pred_test['p1'] * 100)


X_test_pred = pd.concat([ds.X_test.reset_index(drop=True), pred_test], axis=1)
X_test_pred.to_csv('data/processed/X_test_pred_h2o_24_may_binary.csv', index=False)
