...
# filtered_names = ''.join([w for w in names if not w in punctuations])
# filtered_names=''.join([c for c in filtered_names if not c.isdigit()])




# names= 'Large Camelopardalis Virgo UGC Tucana KKh Grus UGCA Aquarius Andromeda Antlia IC Leo Cassiopeia Draco Sagittarius Carina Lacerta Boötes Maffei KUG Ursa Segue Fornax Columba KK98 Holmberg ESO NGC Perseus Horologium Reticulum Indus MB Crater Phoenix Pisces FM2000 Triangulum Sculptor Cas M110 Dwingeloo Cetus HIPASS Small Canes Pegasus Eridanus Willman Wolf-Lundmark-Melotte Donatiello Barnards KKR M32 Sextans KKs GR Pictor DDO Hydrus HIZSS Hercules Hydra Coma'
...
X_train=ds.X_train.reset_index(drop=True)
y_train=ds.y_train.reset_index(drop=True)
X_train['y'] = y_train
...
models_path = '/Users/busebalci/Dev/prohack/models/h2o_models'
...
rms = math.sqrt(mean_squared_error(ds.y_val, h2o.as_list(aml.leader.predict(hf_val))))
print(rms)
...
X_test_pred.to_csv('/Users/busebalci/Dev/prohack/data/processed/X_test_pred_23_may_curve_fitted.csv', index=False)
...
full_data_hf = h2o.H2OFrame(full_data)

#Get y_pred from dnn
y_pred= aml.leader.predict(full_data_hf)
