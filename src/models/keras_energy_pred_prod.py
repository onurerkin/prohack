...
from src.features.preprocess import preprocess_train_with_pred_opt_prod
...
model = train(X_train=ds.X_train, y_train=ds.y_train, X_val=ds.X_train, y_val=ds.y_train,
...
prob_df.to_csv('data/processed/prob_df.csv')

X_test = pd.read_csv('data/processed/test_column_names_fixed.csv')
X_test['energy'] = np.array(ds.X_test['energy'])

belows = X_test[X_test['existence_expectancy_index'] < 0.7]
print("Total Energy has to be given to below 0.7: {}".format(5000 * len(X_test) / 890))
print("Total Enery We Gave to below 0.7: {}".format(sum(belows['energy'])))
print(sum(X_test['energy']))



sum(X_test[X_test['energy'] < 95]['energy'])
...
prob_df['energy'] = prob_df['prob_1'] * 100
prob_df.loc[prob_df['prob_1'] > 0.95, 'energy'] = 100
prob_df.loc[prob_df['prob_1'] < 0.05, 'energy'] = 0
